...
1. **Выбор и создание записи**
   - Используем корпоративный vault (например, 1Password → коллекция `Tipsyo`).
   - Создаем запись `Tipsyo Landing / ENV (dev)` и добавляем поля для всех ключей из `.env.example`.
