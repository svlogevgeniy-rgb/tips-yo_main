# Technical specifications — Tipsio (MVP)

**Документ:** Техническое задание (ТЗ)  
**Продукт:** Tipsio (рабочее название Tips’yo) — Bali-first цифровая платформа для чаевых и гостевого опыта в ресторанах, кафе, барах, кофейнях и близких F&B‑заведениях на Бали с последующим расширением на другие сервисные бизнесы (виллы, спа, бьюти).  
...
### 1.1. Концепция
Tipsio создаёт стандарт цифровых чаевых и сервисов «за столом» на Бали. Гость сканирует QR или открывает ссылку, попадает на лендинг Tipsio, выбирает сумму и завершает оплату на хостируемой странице Midtrans (Snap или Payment Link) в привычном методе (карта, Google Pay, QRIS/e-wallet). Деньги поступают на счёт заведения, платформа прозрачно рассчитывает распределение и даёт персоналу и управляющим историю начислений и аналитику.

### 1.2. Цель проекта
Обеспечить запуск Bali-first MVP цифровых чаевых через Midtrans (Snap Hosted Checkout, Payment Link и Core API) так, чтобы гости легко оставляли чаевые без наличных, сотрудники видели честные начисления, а владельцы заведений управляли правилами распределения и аналитикой без изменения текущей платёжной инфраструктуры.

### 1.3. Показатели успеха MVP
**Продуктовые метрики (см. `docs/product/vision.md`):**
- Подключено X–Y активных F&B‑заведений на Бали с регулярными транзакциями чаевых.
- Не менее N транзакций чаевых в месяц на активное заведение, рост средней суммы чаевых и доли гостей, оставляющих чаевые.
- Количество транзакций, средний размер, доля чаевых от числа закрытых счетов — в аналитике кабинета.
- Активность персонала: доля сотрудников, заходящих в личный кабинет хотя бы раз в неделю.
- NPS заведений по продукту находится на целевом уровне.

**Технические SLA:**
- Время от открытия лендинга до подтверждения оплаты в Midtrans notification ≤ 10 секунд при нормальной сети.  
...
- Доступность лендинга и API Tipsio ≥ 99.5% в рабочее время заведений.
