...
2. Эталонный `.env` формируется DevOps и хранится в защищённом vault (1Password, Bitwarden, SOPS).
3. При онбординге разработчик получает доступ к vault и копирует `.env` локально. Файл не коммитится (см. `.gitignore`).
